...
if ( math.abs(sx) < 64 ) then EntitySetTransform( entity_id, x, y, r, sx + 0.01 * sign(sx), sy + 0.01 * sign(sy) ) end
...
if ( count < tcount ) and ( countcomp ~= 0 ) then
	...
	if ( ac ~= nil ) then
		ComponentSetValue2( ac, "attack_ranged_entity_file", "data/entities/projectiles/deck/crazy_sausage.xml" )
	end
	...
	EntitySetTransform( entity_id, x, y, r, sx * 0.01, sy * 0.01 )
	...
	for i=count,tcount-1 do
		if ( c ~= nil ) then
			...
			pr = ComponentObjectGetValue2( c, "damage_multipliers", "drill" )
			pr = pr - 0.1
			ComponentObjectSetValue2( c, "damage_multipliers", "drill", pr )
		end
