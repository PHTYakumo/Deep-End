...
local hand_comp = GetUpdatedComponentID()
local pos_x, pos_y = EntityGetFirstHitboxCenter( entity_id )

local range = 210
local projectile_velocity = 70
local scatter = 0.3
...
shoot_projectile( root_id, "data/entities/projectiles/orb_shine_mysterious.xml", pos_x, pos_y, vel_x, vel_y)

-- delay randomly so that multiple hands shoot at different times
if hand_comp ~= nil and hand_comp ~= 0 then ComponentSetValue2( hand_comp, "execute_every_n_frame", 75 + Random(15) ) end
