...
local radius = 24 -- much smaller than 96
...
if ( who_shot ~= nil ) and ( comp ~= nil ) then
	for i,projectile_id in ipairs(projectiles) do
		if ( projectile_id ~= root_id ) and ( projectile_id ~= entity_id ) and ( EntityHasTag( projectile_id, "spells_to_power_target" ) == false ) and ( EntityHasTag( projectile_id, "projectile_not" ) == false ) then
			...
			if ( comp2 ~= nil ) then
				...
				if ( who_shot == who_shot2 ) and ( who_shot ~= NULL_ENTITY ) then delete = true end
			end
	...
	damage = math.max( damage, 0.004 ) + math.max( count * 0.08, 0 )
	expdamage = math.max( expdamage, 0.004 ) + math.max( expcount * 0.08, 0 )
	exprad = math.min( 120, math.floor( exprad + math.log( math.max( totalcount, 0.004 ) * 10.5 ) ) )
	...
	ComponentObjectSetValue2( comp, "config_explosion", "particle_effect", false )
	...
	edit_component( effect_id, "ParticleEmitterComponent", function(comp3,vars)
		local part_min = math.min( math.floor( totalcount * 0.25 ), 20 )
		local part_max = math.min( math.floor( totalcount * 0.5 ) + 1, 40 )
