...
if #EntityGetInRadiusWithTag( pos_x, pos_y, 60, "glue") > 10 then
...
if target2 ~= target and ( not EntityHasTag( target2, "glue_NOT" ) ) then
	component_write( EntityGetFirstComponent( children[2], "VariableStorageComponent" ), { value_int = target2 } )
else
	SetRandomSeed(pos_y, pos_x + GameGetFrameNum())
	target2 = EntityGetRootEntity(random_from_array(targets))
