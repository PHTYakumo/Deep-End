...
local projectiles = EntityGetInRadiusWithTag( x, y, 72, "projectile" )
...
if ( #projectiles > 0 ) then
	for i,projectile_id in ipairs(projectiles) do	
		...
		if ( distance < 64 ) and ( entity_id ~= projectile_id ) and ( not EntityHasTag( projectile_id, "ultra" ) ) and ( not EntityHasTag( projectile_id, "thunder" ) ) then
