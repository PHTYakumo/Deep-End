...
local action_count = math.min( Random(2,4), tonumber(deck_capacity) )
local gun_action = get_random_from( gun.actions )


if ModSettingGet( "DEEP_END.FESTIVAL_EVENTS" ) and ( month == 10 and day == 31 ) or ( month == 11 and day == 1 ) then
	for i=1,7 do AddGunActionPermanent( entity_id, "TINY_GHOST" ) end
else
	for i=1,action_count do AddGunAction( entity_id, gun_action ) end
end
