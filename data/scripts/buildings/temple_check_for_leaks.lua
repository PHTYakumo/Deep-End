...
-- the print-decoration of null-room hasn't been modified yet

function material_area_checker_failed( pos_x, pos_y )

	if( GlobalsGetValue( "TEMPLE_PEACE_WITH_GODS" ) == "1" ) then
		GamePrintImportant( "$logdesc_temple_peace_temple_break", "", "data/ui_gfx/decorations/peace_temple_break.png" )
	...
	if( GlobalsGetValue( leak_name ) ~= "1" ) then
		if tonumber(GlobalsGetValue("STEVARI_DEATHS", 0)) < 3 then
			GamePrintImportant( "$logdesc_temple_spawn_guardian", "", "data/ui_gfx/decorations/angered_the_gods.png" )
		else
			GamePrintImportant( "$logdesc_gods_are_very_angry", "", "data/ui_gfx/decorations/angered_the_gods.png" )
