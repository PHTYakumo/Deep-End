...
function generate_shop_item( x, y, cheap_item, biomeid_, is_stealable )
	...
	if biomeid >= 10 then
		price = price * 2.5
		cardcost = cardcost * 2.5
	end

	if cardcost < 500 then cardcost = cardcost * 0.5 + 5 end
	cardcost = math.max( math.floor( cardcost ), 1 )
	...
	EntityAddComponent( eid, "SpriteComponent", { 
		...
		offset_y=tostring( Random( 1, 100 ) * 0.025 + 25 ), 
		update_transform="1" ,
	...
	if is_stealable then stealable_value = "1" end
	...
	EntitySetTransform( eid, x, y, Random(-100,100) * 0.00061, 0.83, 0.83 )
	EntityApplyTransform( eid, x, y, Random(-100,100) * 0.00061, 0.83, 0.83 )
...
function generate_shop_wand( x, y, cheap_item, biomeid_ )
	...
	EntitySetTransform( eid, x, y, 0, 0.872, 0.872 )
	EntityApplyTransform( eid, x, y, 0, 0.872, 0.872 )
