...
function spawn_all_shopitems( x, y )
	...
	SetRandomSeed( x, y )
	local count = tonumber( GlobalsGetValue( "TEMPLE_SHOP_ITEM_COUNT", "5" ) ) + 2
	local width = 144
	local item_width = width / count
	local sale_item_i = Random(1, count)
	local sale_item_1 = Random(1, sale_item_i)
	local sale_item_2 = Random(sale_item_i, count)

	if( y > 4000 ) and ( y < 16000 ) then
		for i=1,count do
			if( i == sale_item_i ) then
				generate_shop_wand( x + (i-1)*item_width - 12, y, true )
			else
				generate_shop_wand( x + (i-1)*item_width - 12, y, false )
			end
		end
	else	
		for i=1,count do
			local goods_type = "data/biome_impl/temple/shop_second_row"
			if 2 >= Random(1, count) then goods_type = goods_type .. "_stealable" end

			LoadPixelScene( goods_type .. ".png", "data/biome_impl/temple/shop_second_row_visual.png", x + (i-1)*item_width - 8, y-22, "", true )
			
			goods_type = false
			if i == sale_item_1 or i == sale_item_2 then goods_type = true end
			
			generate_shop_item( x + (i-1)*item_width - 12, y, goods_type, nil, true )
...
function spawn_worm_deflector( x, y )
	...
	PhysicsApplyTorque( EntityLoad( "data/entities/buildings/physics_worm_deflector_crystal.xml", x, y + 4 ), 666 )
...
function spawn_stage_bonus(x, y)
	...
	if ( newgame_n == 0 ) then
		...
		if ( y>1600 ) and ( y<2000 ) then
			card = "BURST_4"
		elseif ( y>4675 ) and ( y<5075 ) then
			card = "LIGHT"
		elseif ( y>8750 ) and ( y<9150 ) then
			card = "POWERDIGGER"
		elseif ( y>13900 ) and ( y<14300 ) then
			card = "SOILBALL"
		elseif ( y>19500 ) and ( y<19900 ) then
			card = "DE_LINE_BREAKER"
		elseif ( y>25600 ) and ( y<26000 ) then
			card = "X_RAY"
		elseif ( y>32300 ) and ( y<32700 ) then
			card = "TELEPORTATION_FIELD"
		end

		local eid = CreateItemActionEntity( card, spx, spy )
		...
		EntitySetTransform( eid, x, y, 0, 1.8, 1.8 )
		EntityApplyTransform( eid, x, y, 0, 1.8, 1.8 )
	end
end

function spawn_fish(x, y)
	EntityRemoveTag( EntityLoad( "data/entities/buildings/wallchest.xml", x-260, y+5 ), "homing_target" )
	EntityAddTag( EntityLoad( "data/entities/animals/super_fish.xml", x-25, y+15 ), "forgeable" )

	if GameHasFlagRun( "deep_end_wealth_perk_curse" ) then return end
	
	--[[
	local chef = EntityLoad( "data/entities/misc/miner_chef.xml", x-180, y-45 )
	EntityAddRandomStains( chef, CellFactory_GetType("magic_liquid_berserk"), 66 )

	chef = EntityLoad( "data/entities/animals/fox.xml", x+240, y-25 )
	EntityAddRandomStains( chef, CellFactory_GetType("magic_liquid_charm"), 66 )
	]]--

	spawn_stage_bonus(x, y)

	if y < 2560 then return end
	
	local f = math.ceil( y * 0.0003 )
	
	for i=1,f do EntityLoad( "data/entities/animals/fish.xml", x, y ) end
end
