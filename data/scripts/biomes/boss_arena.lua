...
function spawn_items( x, y )
	...
	EntityLoad( "data/entities/animals/boss_centipede/chunk_stain.xml", x, y )
	EntityLoad( "data/entities/animals/boss_centipede/chunk_stain.xml", x - 512, y ) 
end
...
function spawn_hp( x, y )
	...
	EntityLoad( "data/entities/misc/bloody_stain.xml", x, y )
	EntityLoad( "data/entities/misc/bloody_stain.xml", x-128, y )
end
...
function spawn_all_shopitems( x, y )
	...
	local sale_item_1 = Random(1, count)
	local sale_item_2 = Random(1, sale_item_1)
	sale_item_1 = Random(sale_item_1, count)

	for i=1,count do
		LoadPixelScene( "data/biome_impl/temple/shop_second_row_stealable.png", "data/biome_impl/temple/shop_second_row_visual.png", x + (i-1)*item_width - 8, y-22, "", true )

		local goods_type = false
		if i == sale_item_1 or i == sale_item_2 then goods_type = true end
		
		generate_shop_item( x + (i-1)*item_width - 18, y, goods_type, nil, true )
...
function spawn_lamp(x, y)
	for i=1,3 do
		spawn(g_lamp,x+5*(i-2),y,0,10)
	end
end

function spawn_lamp_long(x, y)
	for i=1,3 do
		spawn(g_lamp,x+5*(i-2),y,0,15)
	end
end
...
function spawn_perk_reroll( x, y )
	...
	EntityLoad( "data/entities/items/pickup/perk_reroll.xml", x-150, y )
