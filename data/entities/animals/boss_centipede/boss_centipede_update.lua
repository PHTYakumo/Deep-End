...
function init_boss()
	...
	if init_comp and ComponentGetValue2(init_comp, "value_bool") == false then
		...
		if damagemodel_comp ~= nil then
			...
			ComponentSetValue2( damagemodel_comp, "blood_sprite_directional", "" )
			ComponentSetValue2( damagemodel_comp, "blood_sprite_large", "" )

			ComponentObjectSetValue2(damagemodel_comp, "damage_multipliers", "explosion", 0.2)
			...
			if orbcount >= 0 then
				...
				ComponentObjectSetValue2(damagemodel_comp, "damage_multipliers", "slice", 0.25)
				ComponentObjectSetValue2(damagemodel_comp, "damage_multipliers", "projectile", 0.25)
				ComponentObjectSetValue2(damagemodel_comp, "damage_multipliers", "electricity", 0.25)
			...
			if orbcount >= 4 then
				...
				ComponentObjectSetValue2(damagemodel_comp, "damage_multipliers", "curse", 0.1)
				ComponentObjectSetValue2(damagemodel_comp, "damage_multipliers", "holy", 0.1)
			end
		end
...
function phase_circleshot()
	move_to_reference()
	boss_wait(40) -- a brief pause if shield was previously on
...
function circleshot_aggro()
	...
	local branches = clamp( 4 + 2 * orbcount, 3, 18 )
	local speed = 120
	local frame = GameGetFrameNum()
	...
	GameCreateParticle( "material_darkness", pos_x, pos_y, 50, 0, -20, true, false )
...
function homingshot()
	...
	shoot_projectile( this, "data/entities/animals/boss_centipede/orb_homing.xml", pos_x, pos_y, -vel_y, vel_x )
	shoot_projectile( this, "data/entities/animals/boss_centipede/orb_homing.xml", pos_x, pos_y, vel_y, -vel_x )
	shoot_projectile( this, "data/entities/animals/boss_centipede/orb_polymorph_bouncy.xml", pos_x, pos_y - 10, 0, -50 )
	shoot_projectile( this, "data/entities/animals/boss_centipede/orb_polymorph_bouncy.xml", pos_x + 5, pos_y, 30, 20 )
	shoot_projectile( this, "data/entities/animals/boss_centipede/orb_polymorph_bouncy.xml", pos_x - 5, pos_y, -30, 20 )
end
...
function polymorphshot()
	...
	local rept = 16
	local space  = math.floor(240 / rept)
	local speed  = 200
	...
	for i=1,rept do
		local vel_x = math.cos( math.rad(angle) ) * speed
		local vel_y = math.sin( math.rad(angle) ) * speed
		shoot_projectile( this, "data/entities/animals/boss_centipede/orb_polymorph_bouncy.xml", pos_x, pos_y, vel_x, vel_y )
		angle = angle + space
	end
...
function spawn_minion()
	...
	EntityLoad( "data/entities/animals/boss_centipede/minion/berserkspirit.xml", x+10, y+5 )
	EntityLoad( "data/entities/animals/boss_centipede/minion/confusespirit.xml", x+5, y-10 )
	EntityLoad( "data/entities/animals/boss_centipede/minion/slimespirit.xml", x-10, y-5 )
	EntityLoad( "data/entities/animals/boss_centipede/minion/weakspirit.xml", x-5, y+10 )
...
function set_logic_state( state )
	...
	if (ai ~= nil) then
		old = ComponentGetValue2( ai, "state" )
	end
	...
	if ( comp ~= nil ) then
		...
		if ( hp <= 0.0 ) then
			...
			for i = 1,40 do
				...
				GameCreateParticle( "material_darkness", x + rand(), y + rand(), 10, i * 5.5, i * 5.5, true, false )
				if i > 20 then
