...
if ( sprite ~= nil ) then
	...
	elseif ( status == 3 ) then
		...
		if damagemodels ~= nil then for i,damagemodel in ipairs(damagemodels) do
			...
			ComponentSetValue2( damagemodel, "max_hp", math.min( max_hp + hp, max_hp * 2, 32768000 ) )
			ComponentSetValue2( damagemodel, "hp", math.min( max_hp, hp * 2, 32768000 ) )
		end end
	...
	if status > 0 then
		shoot_projectile( entity_id, "data/entities/animals/boss_meat/orb_big_homing.xml", x, y, 0, 0 )
