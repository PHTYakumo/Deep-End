...
local count = 25
local angle = math.pi / 0.008
SetRandomSeed( pos_x * vel_x, pos_y * vel_y )

for i=1,count do
	local px = pos_x + vel_x * 0.01
	local py = pos_y + vel_y * 0.01
	local vx = math.cos( angle*(i-13) ) * vel_x - math.sin( angle*(i-13) ) * vel_y 
    local vy = math.sin( angle*(i-13) ) * vel_x + math.cos( angle*(i-13) ) * vel_y
	shoot_projectile_from_projectile( entity_id, "data/entities/animals/boss_alchemist/elecbeam.xml", px + Random( -90, 90 ), py + Random( -20, 20 ), vx, vy )
end
