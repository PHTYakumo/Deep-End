...
for i=1,count do
	local px = pos_x + vel_x * 0.06
	local py = pos_y + vel_y * 0.06
    local vx = math.cos( angle*(i-5) ) * vel_x - math.sin( angle*(i-5) ) * vel_y 
    local vy = math.sin( angle*(i-5) ) * vel_x + math.cos( angle*(i-5) ) * vel_y
	shoot_projectile_from_projectile( entity_id, "data/entities/animals/boss_alchemist/grenade_blue.xml", px, py, vx, vy )
end
