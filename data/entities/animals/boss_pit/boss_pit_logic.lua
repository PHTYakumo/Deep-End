...
local players = EntityGetInRadiusWithTag( x, y, 1024, "player_unit" )
...
if ( state == 1 ) then
	if ( #p > 0 ) then
		...
		local spells = { "sea_acid_small", "sea_lava_small", "circle_bad" }
		local rnd = Random( 1, #spells )
		local path = "data/entities/animals/boss_pit/spells/" .. spells[rnd] .. ".xml"
...
elseif ( state == 4 ) or ( state == 6 ) or ( state == 8 ) then
	if ( Random( 1, 10 ) == 5 ) or ( 1 == 1 ) then
		...
		if( pathfinding_frames_stuck > 160 ) then
			...
			local arc = math.pi / 4
			local offset = math.pi * ( Random( 1, 16 ) * 0.1 )
			
			for i=1,4 do
				for j=1,6 do
					local radius = i^0.6 * 6 * ( 6 + (-1)^j )

					local vel_x = math.cos( arc * j + offset ) * radius
					local vel_y = -math.sin( arc * j + offset ) * radius

					shoot_projectile( entity_id, "data/entities/projectiles/deck/hamis.xml", x-vel_y, y-vel_x, vel_x, vel_y )
				end
