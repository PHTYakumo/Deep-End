...
if is_called == "bh" then
	...
	if ( sprnd <= 8 ) then
		EntityLoad( "mods/deep_end/files/custom_materials/black_hole.xml", x + Random( -10, 10 ), y + Random( -10, 10 ) )
	elseif ( sprnd <= 16 ) then
		EntityLoad( "mods/deep_end/files/custom_materials/white_hole.xml", x + Random( -10, 10 ), y + Random( -10, 10 ) )
	end
end
